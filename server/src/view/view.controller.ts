...
export class ViewController {
  ...
  @Post()
  async upsertView(@Body() dto: UpsertViewDto): Promise<View> {
    return this.viewService.upsertView(dto);
  }

  @Put(':id')
  async updateView(@Param('id') id: string, @Body() dto: UpsertViewDto): Promise<View> {
    // For PUT, we'll use the same upsert logic but ensure the id matches
    const updatedDto = { ...dto, parentId: id };
    return this.viewService.upsertView(updatedDto);
  }

  @Get(':id')
  async getView(@Param('id') id: string): Promise<View | null> {
    return this.viewService.getView(id);
  }

  @Get('snapshot/:snapshotId')
  async getViewsBySnapshot(@Param('snapshotId') snapshotId: string): Promise<View[]> {
