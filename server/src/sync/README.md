...
7. **Transform & Write**:
   - Column mappings are applied via `transformRecordAsync`, which supports optional transformers on each mapping (see [Transformers](#transformers) below).
...
- `LookupTools` for FK resolution (e.g., resolving a foreign key to a destination ID)
...
| [transformers.ts](transformers.ts) | Transformer definitions and `LookupTools` for FK resolution |
