    ...
    if isinstance(error, ModelHTTPError):
        if error.status_code == 403 and "Key limit exceeded" in str(error):
            ...
            return HTTPException(
                ...
                detail="Openrouter ey limit exceeded. Manage it using https://openrouter.ai/settings/keys",
            )
        elif error.status_code == 401 and "Connection error." in str(error):
            # This one I haven't seen in practice.
            return HTTPException(
                status_code=error.status_code,
                detail="Openrouter connection error. Please retry in a moment and contact support if the problem persists.",
            )
        elif error.status_code == 401 and "No auth credentials found" in str(error):
            # Happens when an invalid API key is provided.
            return HTTPException(
                status_code=error.status_code,
                detail="Missing or invalid Openrouter API key. Create a new key at https://openrouter.ai/settings/keys and add it to https://scratchpaper.ai/settings",
            )
            ...
            return HTTPException(
                status_code=error.status_code,
                detail="Openrouter user not found or API key disabled. Check: https://openrouter.ai/settings/keys",
            )
        else:
