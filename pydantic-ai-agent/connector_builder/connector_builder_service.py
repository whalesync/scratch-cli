...
class ConnectorBuilderService:
    ...
    def create_session(self, session_id: str, custom_connector_id: str) -> ConnectorBuilderSession:
        """Create a new connector builder session"""
        session = ConnectorBuilderSession(session_id, custom_connector_id)
        self.sessions[session_id] = session
        log_info("Connector builder session created", session_id=session_id, custom_connector_id=custom_connector_id)
        return session
    ...
    def cleanup_inactive_sessions(self, max_age_hours: int = 24):
        ...
        if sessions_to_delete:
            log_info("Cleaned up inactive connector builder sessions", 
    ...
    async def process_connector_builder_request(
        ...
        print(f"🤖 Starting connector builder agent processing for connector: {custom_connector_id}")
        ...
        try:
            ...
            print(f"✅ Custom connector loaded: {custom_connector.get('name', 'Unknown')}")
            ...
            custom_connector_prompt = custom_connector.get("prompt", "No prompt provided")
            custom_connector_api_key = custom_connector.get("apiKey", "No API key provided")
            
            # Format existing functions
            list_tables_function = custom_connector.get("listTables", "Not generated yet")
            fetch_schema_function = custom_connector.get("fetchSchema", "Not generated yet")
            poll_records_function = custom_connector.get("pollRecords", "Not generated yet")
            get_record_function = custom_connector.get("getRecord", "Not generated yet")
            create_record_function = custom_connector.get("createRecord", "Not generated yet")
            update_record_function = custom_connector.get("updateRecord", "Not generated yet")
            delete_record_function = custom_connector.get("deleteRecord", "Not generated yet")
            ...
            poll_records_response = custom_connector.get("pollRecordsResponse", "No poll records response available")
            get_record_response = custom_connector.get("getRecordResponse", "No get record response available")
...
Please generate, test, and save the function as requested."""
            ...
            agent = create_connector_builder_agent(model_name=model, capabilities=capabilities, style_guides=style_guides)
            ...
            try:
                ...
                log_error("Agent processing timeout", custom_connector_id=custom_connector_id, timeout_seconds=30)
            ...
            if not actual_response:
                log_error("No response from agent", custom_connector_id=custom_connector_id)
            ...
            log_error("Agent processing error", custom_connector_id=custom_connector_id, error=str(e))
