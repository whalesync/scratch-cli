...
class ConnectorBuilderRequestDTO(BaseModel):
    ...
    custom_connector_id: str = Field(description="The ID of the custom connector to work with")
    api_token: Optional[str] = Field(default=None, description="API token for Scratchpad server authentication")
    style_guides: Optional[List[Dict[str, str]]] = Field(default=None, description="List of style guides")
    capabilities: Optional[List[str]] = Field(default=None, description="List of selected capabilities")
...
class ConnectorBuilderResponseDTO(BaseModel):
    ...
    response_message: str = Field(description="The response message to show to the user")
    response_summary: str = Field(description="A brief summary of what the agent did")
    request_summary: str = Field(description="A brief summary of what the user requested")
    generated_function: Optional[str] = Field(default=None, description="The generated function code if applicable")
    function_type: Optional[str] = Field(default=None, description="The type of function generated")
...
class ConnectorBuilderSessionDTO(BaseModel):
    ...
    custom_connector_id: str = Field(description="The custom connector ID this session is associated with")
    chat_history: List[Dict[str, Any]] = Field(description="Chat history for this session")
...
class CreateConnectorBuilderSessionResponseDTO(BaseModel):
    ...
    session: ConnectorBuilderSessionSummaryDTO = Field(description="Session information")
    available_capabilities: List[ConnectorBuilderCapabilityDTO] = Field(description="Available capabilities for this session") 