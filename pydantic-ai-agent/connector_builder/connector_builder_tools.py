...
def define_connector_builder_tools(agent, capabilities: Optional[list] = None):
    ...
    @agent.tool
    async def execute_list_tables_tool(
        context: RunContext[ConnectorBuilderRunContext],
        request: ExecuteListTablesRequest,
    ) -> str:
        """
        Execute a listTables function to test if it works correctly.

        This tool calls the Scratch server's execute-list-tables endpoint to test
        ...
        try:
            ...
            # Call the Scratch server's execute-list-tables endpoint
            response = call_scratchpad_api(
                "POST",
                "/rest/custom-connector-builder/execute-list-tables",
                {"functionString": request.function_string, "apiKey": request.api_key},
                api_token=api_token,
            )
    ...
    @agent.tool
    async def save_custom_connector_tool(
        context: RunContext[ConnectorBuilderRunContext],
        request: SaveCustomConnectorRequest,
    ) -> str:
        """
        ...
        try:
            ...
            current_connector = load_custom_connector(
                request.custom_connector_id, api_token
            )
            ...
            # Call the Scratch server's update endpoint
            response = call_scratchpad_api(
                "PUT",
    ...
    @agent.tool
    async def save_custom_connector_with_test_result_tool(
        context: RunContext[ConnectorBuilderRunContext],
        request: SaveCustomConnectorWithTestResultRequest,
    ) -> str:
        """
    ...
    print(
        f"🔧 Connector builder agent has tools: execute_list_tables_tool, save_custom_connector_tool, save_custom_connector_with_test_result_tool"
    )
