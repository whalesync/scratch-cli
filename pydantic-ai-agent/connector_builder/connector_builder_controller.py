...
async def get_session(session_id: str):
    ...
    if session_id not in connector_builder_service.sessions:
        log_warning("Connector builder session not found", session_id=session_id, available_sessions=list(connector_builder_service.sessions.keys()))
...
@router.post("/sessions/{session_id}/messages", response_model=ConnectorBuilderResponseDTO)
async def send_message(session_id: str, request: ConnectorBuilderSessionMessageRequestDTO):
    """Send a message to the connector builder agent"""
    log_info("Connector builder message received", session_id=session_id, message_length=len(request.message), has_api_token=request.api_token is not None, style_guides_count=len(request.style_guides) if request.style_guides else 0, capabilities_count=len(request.capabilities) if request.capabilities else 0)
    print(f"💬 Processing connector builder message for session: {session_id}")
    print(f"📋 Available sessions: {list(connector_builder_service.sessions.keys())}")
    
    if session_id not in connector_builder_service.sessions:
        log_warning("Connector builder session not found for message", session_id=session_id, available_sessions=list(connector_builder_service.sessions.keys()))
        raise HTTPException(status_code=404, detail="Session not found")
    ...
    try:
        ...
        log_info("Connector builder agent processing started", session_id=session_id, custom_connector_id=session.custom_connector_id)
        
        agent_response = await connector_builder_service.process_connector_builder_request(
            message=request.message,
            custom_connector_id=session.custom_connector_id,
        ...
        log_info("Connector builder agent response received", 
                 session_id=session_id, 
                 custom_connector_id=session.custom_connector_id,
        ...
        log_error("Connector builder message processing failed", 
                  session_id=session_id, 
                  custom_connector_id=session.custom_connector_id,
                  error=str(e))
...
async def delete_session(session_id: str):
    ...
    if session_id not in connector_builder_service.sessions:
        log_warning("Connector builder session not found for deletion", session_id=session_id)
        raise HTTPException(status_code=404, detail="Session not found")
    
    connector_builder_service.delete_session(session_id)
    log_info("Connector builder session deleted", session_id=session_id, remaining_sessions=len(connector_builder_service.sessions))
...
async def process_connector_builder_request(request: ConnectorBuilderRequestDTO):
    ...
    log_info("Legacy connector builder request received", 
             custom_connector_id=request.custom_connector_id, 
             message_length=len(request.message), 
             has_api_token=request.api_token is not None)
    
    print(f"🔧 Processing legacy connector builder request for connector: {request.custom_connector_id}")
    print(f"📝 Message: {request.message}")
    
    try:
        # Process with agent
        print(f"🤖 Processing with connector builder agent...")
        log_info("Connector builder agent processing started", custom_connector_id=request.custom_connector_id)
        ...
        log_error("Connector builder request processing failed", 
                  custom_connector_id=request.custom_connector_id, 
                  error=str(e))
