...
from agents.data_agent.model_utils import find_table_by_name, find_record_by_wsId, is_in_write_focus, missing_table_error, missing_field_error, find_column_by_name, find_column_by_id, get_active_table, unable_to_identify_active_field_error, unable_to_identify_active_record_error, unable_to_identify_active_table_error, record_not_in_context_error, not_in_write_focus_error, unable_to_identify_active_snapshot_error
...
def set_field_value_tool_implementation(ctx: RunContext[ChatRunContext], table: TableSpec, wsId: str, column: ColumnSpec, new_value: str) -> str:
    ...
    try:
        ...
        return f"Successfully set the value in the {column.name} field in record {wsId}."
...
def define_set_field_value_tool(agent: Agent[ChatRunContext, ResponseFromAgent], data_scope: Optional[str] = None):
    ...
    if data_scope == 'column':
        ...
        async def set_value_tool(ctx: RunContext[ChatRunContext], new_value: str) -> str:  # type: ignore
            ...
            if not column:
                return unable_to_identify_active_field_error(table, chatRunContext.column_id)

            wsId = chatRunContext.record_id
            
            return set_field_value_tool_implementation(ctx, table, wsId, column, new_value)
