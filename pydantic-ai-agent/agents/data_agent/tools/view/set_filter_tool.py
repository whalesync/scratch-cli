...
def define_set_filter_tool(agent: Agent[ChatRunContext, ResponseFromAgent], capabilities: Optional[List[str]] = None):
    ...
    async def set_filter_tool(ctx: RunContext[ChatRunContext], input_data: SetFilterInput) -> str:  # type: ignore
        ...
        try:
            ...
            log_error("Error setting filter", 
                    table_name=table_name,
                    sql_where_clause=sql_where_clause if 'sql_where_clause' in locals() else None,
                    error=str(e))
            print(f"❌ {error_msg}")
            return error_msg 