...
class AddRecordsToFilterInput(BaseModel):
    ...
    """
    record_ids: List[str] = Field(description="List of record IDs (wsId) to add to the filter")


def define_add_records_to_filter_tool(agent: Agent[ChatRunContext, ResponseFromAgent], capabilities: Optional[List[str]] = None):
