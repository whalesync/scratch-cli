...
from agents.data_agent.model_utils import find_record_by_wsId, is_in_write_focus, missing_field_error, find_column_by_name, find_column_by_id, get_active_table, unable_to_identify_active_table_error, unable_to_identify_active_record_error, unable_to_identify_active_field_error, unable_to_identify_active_snapshot_error, record_not_in_context_error, not_in_write_focus_error
...
def append_field_value_tool_implementation(ctx: RunContext[ChatRunContext], table: TableSpec, wsId: str, column: ColumnSpec, value: str) -> str:
...
def define_append_field_value_tool(agent: Agent[ChatRunContext, ResponseFromAgent], data_scope: Optional[str] = None):
    """Append a value to a field in a record in a table in the active snapshot."""
    ...
    if data_scope == 'column':
        ...
        async def append_value_tool(ctx: RunContext[ChatRunContext], value: str) -> str:  # type: ignore
            ...
            return append_field_value_tool_implementation(ctx, table, wsId, column, value)
