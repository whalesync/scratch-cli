...
# Connects to the scratch-git HTTP backend service via IAP tunnel and forwards port 3101 locally
# Prerequisites:
#   - Install gcloud CLI and authenticate
# To use:
#   1. Run this script for the environment you want to access: 'test', 'staging', or 'production'
#      > ./connect_to_git_http_backend.sh test
#   2. Access the HTTP backend service at 127.0.0.1:3101
...
echo "Starting SSH tunnel to scratch-git HTTP backend for $ENVIRONMENT..."
...
while ! lsof -iTCP:$LOCAL_PORT -s tcp:listen >/dev/null; do
  ...
  if [ $counter -ge $timeout ]; then
    ...
    echo "Failed to set up SSH tunnel to scratch-git HTTP backend!"
    exit 1
  fi
done

echo "SSH tunnel started. HTTP backend service available at $LOCAL_HOSTNAME:$LOCAL_PORT"
