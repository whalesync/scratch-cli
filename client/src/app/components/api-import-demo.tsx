...
import { Box, Button, Group, Modal, Stack, Table, Text, Textarea, ActionIcon, TextInput } from '@mantine/core';
import { ArrowRight, MagnifyingGlass, Plus, Sparkle, Trash } from '@phosphor-icons/react';
...
export const ApiImport: FC = () => {
  ...
  const getCellContent = useCallback((cell: Item): GridCell => {
    ...
    return { kind: GridCellKind.Text, data: displayValue, displayData: displayValue, allowOverlay: true, readonly: false };
  }, [result, columns]);
  ...
  const handleMappingChange = (id: string, field: 'destination' | 'source', value: string) => {
    setMappings(prev => prev.map(m => m.id === id ? { ...m, [field]: value } : m));
  };
  
  const addMappingRow = () => {
    setMappings(prev => [...prev, { id: `mapping-${nextMappingId.current++}`, destination: '', source: '' }]);
  };
  ...
  return (
    <Stack gap="md">
      ...
      <Stack gap="sm">
        <Text size="lg" fw={600}>1. Configure API Request</Text>
        <Group align="flex-start" gap="md">
          <Stack style={{ flex: 1 }} gap="sm">
            <Stack gap="xs">
              <Text size="sm" fw={500}>Generate fetch parameters with AI (optional)</Text>
              <Textarea
              ...
              />
              <Button leftSection={<Sparkle size={16} />} onClick={handleAiGenerate} loading={aiGenerating} disabled={!aiPrompt.trim()} size="sm" variant="light">
                Generate Parameters
              </Button>
            </Stack>
            <Textarea
            ...
            />
            <Button onClick={handleFetch} disabled={!curl.trim()} leftSection={<ArrowRight size={16} />} style={{ width: '140px' }}>
              Fetch Data
            </Button>
          </Stack>
          <Stack style={{ flex: 1 }}>
      ...
      {response && (
        <Stack gap="sm">
          <Text size="lg" fw={600}>2. Select Record Array Path</Text>
          <Table>
            <Table.Tbody>
              <Table.Tr>
                <Table.Td style={{ width: '200px' }}><Text size="sm" fw={500}>Path to records array:</Text></Table.Td>
                <Table.Td>
                  <Group gap="xs">
                    <TextInput
          ...
          {recordArrayPath && (
            <Stack gap="xs">
              <Text size="sm" fw={500}>Preview of selected array:</Text>
              {(() => {
                try {
                  const arrayData = recordArrayPath === '.' ? response : _.get(response, recordArrayPath);
                  return Array.isArray(arrayData) ? <Text size="sm" c="green">✓ Found array with {arrayData.length} items</Text> : <Text size="sm" c="red">✗ Path does not point to an array</Text>;
                } catch {
                  return <Text size="sm" c="red">✗ Invalid path</Text>;
                }
              })()}
            </Stack>
          )}
      ...
      {response && recordArrayPath && (
        <Stack gap="sm">
          <Text size="lg" fw={600}>3. Map Fields</Text>
          <Table>
            <Table.Thead>
              <Table.Tr>
          ...
          <Button leftSection={<Plus size={16} />} onClick={addMappingRow} variant="light" style={{ alignSelf: 'flex-start' }}>
            Add Row
          </Button>
        </Stack>
      )}
      ...
      {response && recordArrayPath && (
        <Stack gap="sm">
          <Text size="lg" fw={600}>4. Apply Mapping</Text>
          <Button onClick={handleApplyMapping} disabled={!response || mappings.length === 0} leftSection={<ArrowRight size={16} />} style={{ width: '140px' }}>
            Apply Mapping
          </Button>
          {result && result.length > 0 && (
            <Stack gap="xs">
              <Group justify="space-between" align="center">
                <Text size="sm" fw={500}>Mapped Data Preview:</Text>
                <Group gap="xs">
                  <Button size="xs" variant="outline" color="red" onClick={() => onDelete(gridSelection!)} disabled={!gridSelection?.rows || result.length === 0}>Delete Selected</Button>
                  <Button size="xs" variant="outline" color="red" onClick={() => setResult([])} disabled={!result || result.length === 0}>Clear All</Button>
                </Group>
              </Group>
      ...
      {result && result.length > 0 && (
        <Stack gap="sm">
          <Text size="lg" fw={600}>5. Run Import</Text>
          <Button onClick={runImport} loading={importing} leftSection={<ArrowRight size={16} />} style={{ width: '140px' }}>
            Run Import
          </Button>
        </Stack>
      )}
      ...
      <Modal opened={pathModalOpened} onClose={() => setPathModalOpened(false)} title="Select API Response Path" size="lg">
        <Stack gap="sm">
          <Text size="sm" c="dimmed">Click on a path to use it for the selected field:</Text>
      ...
      <Modal opened={recordArrayPathModalOpened} onClose={() => setRecordArrayPathModalOpened(false)} title="Select Array Path" size="lg">
        <Stack gap="sm">
          <Text size="sm" c="dimmed">Click on a path to select the array containing your records:</Text>
          <Stack gap="xs" style={{ maxHeight: '400px', overflowY: 'auto' }}>
